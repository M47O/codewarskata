...
export function splitTheBill(x: { [k: string]: number }): { [k: string]: number } {
  const totalOwed = Object.values(x).reduce((acc, cv) => acc + cv, 0) / Object.values(x).length;
  const personOwes = Object.values(x).map(amountPaid => Number((amountPaid - totalOwed).toFixed(2)));
  
  const result: { [k: string]: number } = {};
  Object.keys(x).forEach((person, index) => {
    result[person] = personOwes[index];
  })
